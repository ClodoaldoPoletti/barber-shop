...
function monta_calendario(p_data, p_marcador) {
    ...
    let data_atual = new Date(p_data);
    ...
    let mes_anterior = data_atual.getMonth() - 1;
    let ano_anterior = data_atual.getFullYear() - 1;
    ...
    //let mes_subsequente = data_atual.getMonth() + 1;
    //let ano_subsequente = data_atual.getFullYear() + 1;

    let quantidade_dias_mes_anterior = new Date(ano_anterior, mes_anterior + 1, 0).getDate();
    let quantidade_dias_mes_atual = new Date(ano_atual, mes_atual + 1, 0).getDate();
    ...
    //console.log("Mês atual: " + mes_atual + " Ano atual: " + ano_atual);

    if (mes_servidor == mes_atual && ano_servidor == ano_atual) {
        dia_atual = dia_servidor;
        marcador = 1;
    }
    ...
    //console.log("Mês atual: " + mes_descricao_lista[mes_atual] + " - dias: " + quantidade_dias_mes_atual);
    ...
    let dia_primeiro_do_mes = new Date(ano_atual, mes_atual, 1);
    ...
    for (a = 0; a < 6; a++) {
        ...
        for (b; b < 7; b++) {
            ...
            if (c <= quantidade_dias_mes_atual) {
                calendario[a][b] = c;
                c++;
            }
        }   
    ...
    let mes_descricao = mes_descricao_lista[mes_atual];
    let mes_ano = mes_descricao + ' de ' + ano_atual;
    document.getElementById("mes_ano").innerHTML = mes_ano;


    // Identifica o dia da semana
    //let dia_da_semana = data_atual.getDay();
    ...
    for (let i = 0; i < 6; i++) {
        for (let j = 0; j < 7; j++) {
            ...
            if (calendario[i][j] != x) {
                ...
                if (calendario[i][j] == dia_atual && marcador == 1) {
                    li_tag.classList.add("section__schedule__days-content-day-mark");
                } else {
            ...
            let li_conteudo = document.createTextNode(calendario[i][j]);
            
            li_tag.appendChild(li_conteudo);
...
function direcao(opcao) {
    ...
    //if (mes_data_servidor == mes_atual && ano_data_servidor == ano_atual) {
    //    marcador = 1;
    //}
